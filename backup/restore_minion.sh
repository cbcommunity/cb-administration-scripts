...
update_all_slaves_with_new_slave () {
    ...
    do
        ...
        if [ $_slave == 1 ] && [ ! -z "$_slave_host" ] && [ ! -z "$_slave_user"  ]; then
            ...
            #remote_exec $last_conn "rm -rf /var/cb/.erlang.cookie"
...
generate_init_conf(){
    ...
    remote_exec $_master_conn "service cb-pgsql start"
...
EOF
    ...
    remote_exec $_master_conn "service cb-pgsql stop"
...
EOF
    ...
    echo "$_slave_host/$_new_slave_host"
    ...
    remote_exec $_master_conn "service iptables stop"
    remote_exec $_master_conn "sed -i 's/$OLD_IP/$NEW_IP/g' /etc/sysconfig/iptables"
    remote_exec $_master_conn "service iptables start"
    color_echo "--- Done"
...
init_slave_node () {
    ...
    remote_copy $_master_conn "$_local_backup_dir/cluster.conf" "/etc/cb/" 0
